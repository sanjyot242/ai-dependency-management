...
router.get('/:id', async (req, res) => {
  try {
    ...
    //return res.json(user);
     const repos = await Repository.find({ id })
     res.json(repos, user);
...
router.post('/sync',async(req,res)=>{
  try{
    ...
    if(!user|| !user.githubToken){
      return res.status(400).json({error:'User not found or missing GitHub token'});
    }
    ...
    await Promise.all(
      githubRepos.map(async(repo)=>{
        ...
        await Repository.findOneAndUpdate(filter,update,{upsert:true,new:true})
      })
    );
